...
fast: *.tex
	latexmk -xelatex $(DIPLOMA_REPORT_PDF)
	# mv $(DIPLOMA_REPORT_PDF).pdf $(DIPLOMA_REPORT_PDF)-`date +'%m-%d-%H-%M-%S'`.pdf

# practice: *.tex
# 	latexmk -pdf -pdflatex="pdflatex" $(PRACTICE_REPORT_PDF)
...
.PHONY: clean
clean:
	$(RM) *.aux *.log *.out *.toc *.gz *.gz\(busy\) *.blg *.bbl *.fls *.fdb_latexmk $(DIPLOMA_REPORT_PDF).pdf $(TASK_PDF).pdf $(TITLE_PDF).pdf $(PRACTICE_REPORT_PDF).pdf $(COURSE_REPORT_PDF).pdf
